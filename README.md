    ...
    <style>
        * {
        ...
        .registered-btn:hover {
            background: linear-gradient(135deg, #ea580c, #c2410d);
            transform: translateY(-3px);
...
<body>
    ...
    <main>
        ...
        <div class="auth-buttons">
            ...
            <button class="auth-btn registered-btn">
                <i class="fas fa-check-circle"></i> Já sou cadastrado
            </button>
        </div>
    ...
    <script>
        ...
        // Ações dos botões de autenticação
        document.querySelectorAll('.auth-btn').forEach(btn => {
            btn.addEventListener('click', function() {
                const text = this.textContent.trim();
                alert(`Você clicou em: "${text}"\n\nEm breve, isso levará você à página correspondente.`);
            });
        });
    </script>
